...
function makeUdpTransport(options, callback) {
  ...
  function open(remote, error) {
    return {
      send: function(m) {
        var s = stringify(m);
        socket.send(Buffer.from(s, 'ascii'), 0, s.length, remote.port, remote.address);          
      },
      protocol: 'UDP',
      release : function() {}
    }; 
...
function makeTransport(options, callback) {
  ...
  function wrap(obj, target) {
    return Object.create(obj, {send: {value: function(m) {
      if(m.method) {
        m.headers.via[0].host = options.publicAddress || options.address || options.hostname || localAddrForAddr(target.address) || os.hostname();
...
function createInviteClientTransaction(rq, transport, tu, cleanup, options) {
  ...
  var completed = {
    enter: function(rs) {
      ack.headers.via[0].params.branch = generateBranch();
  ...
  var timer_m;
  var accepted = { /* aka "confirmed" in the rfc */
    enter: function(rs) {
      ack.headers.via[0].params.branch = generateBranch();
      ack.headers.to=rs.headers.to;
      transport(ack);
